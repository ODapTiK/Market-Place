...
namespace UserService
{
    public class CreateManufacturerUseCaseTests
    {
        ...
        public async Task Execute_ShouldThrowValidationException_WhenManufacturerDTOIsInvalid()
        {
            ...
            _validatorMock.Setup(v => v.ValidateAsync(manufacturerDTO, CancellationToken.None))
                .ReturnsAsync(new ValidationResult(validationErrors));

            // Act
            Func<Task> act = async () => await _createManufacturerUseCase.Execute(manufacturerDTO);
        ...
        public async Task Execute_ShouldCreateManufacturer_WhenManufacturerDTOIsValid()
        {
            ...
            _manufacturerRepositoryMock.Setup(m => m.CreateAsync(manufacturer, CancellationToken.None))
                .ReturnsAsync(manufacturer.Id);

            // Act
            var result = await _createManufacturerUseCase.Execute(manufacturerDTO);

            // Assert
            result.Should().Be(manufacturer.Id);
            _manufacturerRepositoryMock.Verify(m => m.CreateAsync(manufacturer, CancellationToken.None), Times.Once);
        }
    }
}
