    ...
    public class CreateAdminUseCaseTests
    {
        ...
        public async Task Execute_ShouldCreateAdmin_WhenAdminDTOIsValid()
        {
            ...
            _mapperMock.Setup(m => m.Map<Admin>(adminDTO))
                .Returns(admin);
            _adminRepositoryMock.Setup(a => a.CreateAsync(admin, It.IsAny<CancellationToken>()))
                .ReturnsAsync(adminId);

            // Act
            var result = await _createAdminUseCase.Execute(adminDTO, CancellationToken.None);

            // Assert
            result.Should().Be(adminId);
            _adminRepositoryMock.Verify(a => a.CreateAsync(admin, It.IsAny<CancellationToken>()), Times.Once);
