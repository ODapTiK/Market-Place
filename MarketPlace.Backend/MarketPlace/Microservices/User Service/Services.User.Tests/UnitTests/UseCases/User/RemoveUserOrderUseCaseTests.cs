...
namespace UserService
{
    public class RemoveUserOrderUseCaseTests
    {
        ...
        public async Task Execute_ShouldRemoveOrder_WhenUserIdAndOrderIdAreValid()
        {
            ...
            // Act
            await _removeUserOrderUseCase.Execute(userId, orderId);

            // Assert
            _userRepositoryMock.Verify(u => u.RemoveOrderAsync(user, orderId, CancellationToken.None), Times.Once);
        }
        ...
        public async Task Execute_ShouldThrowValidationException_WhenUserIdIsEmpty()
        {
            ...
            // Act
            Func<Task> act = async () => await _removeUserOrderUseCase.Execute(userId, orderId);
