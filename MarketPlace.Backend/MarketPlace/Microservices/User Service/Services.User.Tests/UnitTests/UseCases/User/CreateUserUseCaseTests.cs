    ...
    public class CreateUserUseCaseTests
    {
        ...
        public async Task Execute_ShouldCreateUser_WhenUserDTOIsValid()
        {
            ...
            _mapperMock.Setup(m => m.Map<User>(userDTO))
                .Returns(user);
            _userRepositoryMock.Setup(u => u.CreateAsync(user, CancellationToken.None))
                .ReturnsAsync(userId);
            ...
            _userRepositoryMock.Verify(u => u.CreateAsync(user, CancellationToken.None), Times.Once);
        }
        ...
        public async Task Execute_ShouldThrowValidationException_WhenUserDTOIsInvalid()
        {
            ...
            // Act
            Func<Task> act = async () => await _createUserUseCase.Execute(userDTO);

            // Assert
            await act.Should().ThrowAsync<FluentValidation.ValidationException>();
            _userRepositoryMock.Verify(u => u.CreateAsync(It.IsAny<User>(), CancellationToken.None), Times.Never);
        }
    }
}
